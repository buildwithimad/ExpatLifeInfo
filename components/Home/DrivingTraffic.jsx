...
export default function DrivingTraffic({ lang, drivingPosts }) {
  const isArabic = lang === "ar";

  // UI Translations
  const t = {
    sectionEyebrow: isArabic ? "التنقل والمركبات" : "Mobility & Transport",
    sectionTitle: isArabic ? "القيادة والمرور" : "Driving & Traffic",
    viewAll: isArabic ? "جميع أخبار المرور" : "All Traffic News",
    defaultCategory: isArabic ? "نقل" : "Transport",
  ...
  // Safety check
  if (!drivingPosts) return null;

  return (
    <section 
      dir={isArabic ? "rtl" : "ltr"} 
      className="py-16 md:py-28 px-5 lg:px-8 bg-white"
    >
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-12 md:mb-16">
          ...
          <Link 
            href={`/${lang}/category/driving-and-traffic`}
            className="group flex items-center gap-2 bg-[#f5f0e8] px-5 py-2.5 rounded-full text-[#0d1a0f] font-bold uppercase tracking-widest text-[0.65rem] transition-colors hover:bg-[#0d1a0f] hover:text-[#f5f0e8] shrink-0"
          >
            {t.viewAll}
        ...
        {/* Dynamic State: Empty vs Populated */}
        {drivingPosts.length === 0 ? (
          
          // Empty State
          <div className="flex flex-col items-center justify-center py-24 bg-[#f5f0e8]/40 rounded-2xl">
            ...
            {drivingPosts.map((post) => (
              <article key={post._id} className="group flex flex-col h-full">
                <Link href={`/${lang}/news/${post.slug}`} className="flex flex-col h-full outline-none">
                  ...
                  <div className="relative w-full aspect-[16/11] overflow-hidden bg-[#f5f0e8] mb-4 sm:mb-5 rounded-xl">
                    <Image 
                      src={post.image} 
                      alt={post.title}
                      fill
                  ...
                  <div className="flex flex-col flex-1">
                    ...
                    {/* Excerpt - Hidden on mobile, line-clamp-2 */}
