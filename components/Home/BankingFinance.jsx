...
export default function BankingFinance({ lang, financePosts }) {
  const isArabic = lang === "ar";

  // UI Translations
  const t = {
    sectionEyebrow: isArabic ? "الذكاء المالي" : "Financial Intelligence",
    sectionTitle: isArabic ? "البنوك والمالية" : "Banking & Finance",
    viewAll: isArabic ? "جميع الأخبار المالية" : "All Finance News",
    defaultCategory: isArabic ? "مالية" : "Finance",
    readMore: isArabic ? "اقرأ المزيد" : "Read More", // Added translation
  };

  // Prevent rendering if there is no data
  if (!financePosts || financePosts.length === 0) return null;
    ...
    >
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-12 md:mb-16">
          ...
          <Link 
            href={`/${lang}/category/banking-and-finance`}
            ...
            <article key={post._id} className="group flex flex-col h-full">
              <Link href={`/${lang}/blog/${post.slug}`} className="flex flex-col h-full outline-none">
                ...
                <div className="flex flex-col flex-1">
                  ...
                  <p className="hidden sm:block text-[#4a5e4d] text-sm leading-relaxed line-clamp-3 mb-4">
                    {post.excerpt}
                  </p>
