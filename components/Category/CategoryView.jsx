...
export default function CategoryView({ category, postsData, lang, slug, currentPage, postsPerPage }) {
  const { posts, total } = postsData;
  const totalPages = Math.ceil(total / postsPerPage);
  const isArabic = lang === "ar";
  
  // Safe extraction helper for title
  const categoryTitle = typeof category.title === "string" ? category.title : category.title?.[lang] || "";
      ...
      <section className="relative h-[48vh] min-h-[380px] w-full bg-[#0d1a0f] overflow-hidden flex items-end">
        ...
        <div className="relative z-10 w-full max-w-[1400px] mx-auto px-5 lg:px-8 pb-12">
          ...
          <h1
            ...
            style={{ fontFamily: "'Georgia', 'Times New Roman', serif", fontSize: "clamp(3rem, 8vw, 7rem)" }}
          >
            {categoryTitle}
          </h1>
        </div>
      ...
      <main className="max-w-[1400px] mx-auto py-16 px-5 lg:px-8">
        ...
        <div className="flex items-baseline justify-between mb-14 pb-4 border-b-2 border-[#0d1a0f]">
          <span className="text-[0.65rem] text-[#0d1a0f]/50 font-black tracking-[0.3em] uppercase">
            {isArabic ? `${total} مقالة` : `${total} Article${total !== 1 ? "s" : ""}`}
          </span>
        ...
        {posts.length === 0 ? (
          <div className="py-40 text-center">
            <p className="text-[#4a5e4d] text-3xl italic" style={{ fontFamily: "'Georgia', serif" }}>
          ...
          <>
            ...
            {currentPage === 1 && posts[0] && (
              <article className="group mb-20">
                <Link href={`/${lang}/blog/${posts[0].slug || ""}`} className="grid grid-cols-1 lg:grid-cols-2 gap-10 outline-none">
                  {/* Image */}
                  ...
                  <div className="flex flex-col justify-center">
                    <p className="text-[#b5651d] text-[0.65rem] font-black uppercase tracking-[0.3em] mb-5">
                      {new Date(posts[0].publishedAt).toLocaleDateString(isArabic ? "ar-SA" : "en-US", { year: "numeric", month: "long", day: "numeric" })}
                    </p>
                    <h2
            ...
            {/* ── Article Grid ──────────────────────────── */}
            <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-x-10 gap-y-16">
                ...
                <article key={post._id} className="group flex flex-col">
                  <Link href={`/${lang}/news/${post.slug || ""}`} className="flex flex-col h-full outline-none">
                    ...
                    <h3
                      ...
                      style={{ fontFamily: "'Georgia', serif", fontSize: "clamp(1.1rem, 1.8vw, 1.35rem)" }}
                    >
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => {
                ...
                if (!isNear) {
                  if (pageNum === 2 || pageNum === totalPages - 1) {
                    return <span key={pageNum} className="w-8 text-center text-[#0d1a0f]/30 text-sm font-bold">…</span>;
                  }
                  return null;
                }
                ...
                return (
                  <Link
                    ...
                    className={`w-10 h-10 flex items-center justify-center text-sm font-black transition-all duration-200 ${
                      isActive ? "bg-[#0d1a0f] text-[#f5f0e8]" : "text-[#0d1a0f]/60 hover:text-[#0d1a0f]"
                    }`}
                  >
                    {pageNum}
                  </Link>
